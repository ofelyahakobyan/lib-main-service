...
class UsersController {
  ...
  static registration = async (req, res, next) => {
    try {
      ...
      if (req.file) {
        await GCS.deleteFile(BUCKET_NAME, fileName).catch((e) =>
          console.log(e),
        );
      }
      next(e);
  ...
  static editProfile = async (req, res, next) => {
    try {
      ...
      if (req.file && req.fileName) {
        const { user } = await serviceUsers('getProfile', {
          userId: +req.userId,
        });
      ...
      const data = await serviceUsers('editProfile', {
        ...req.body,
        userId: +req.userId,
      });
      if (data && oldAvatar) {
        setImmediate(() =>
          GCS.delete(BUCKET_NAME, `users/${oldAvatar.split('/')[4]}`),
        );
      }
